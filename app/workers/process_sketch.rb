...
class ProcessSketchWorker
  def self.call(id)
    ...
    $logger.info "[#{id}] fetching #{sfile.dropbox_path} into #{sfile.local_path}"
    ...
    pages_base_path = File.join('images', sfile.local_path, 'pages')
    ...
    slice_info['pages'].each do |page_config|
      ...
      page_config['artboards'].each do |artboard_config|
        ...
        new_filename_with_path = File.join('images', sfile.local_path, new_filename)
    ...
    thumbnail_paths.each do |path|
      ...
      `convert -thumbnail 300 #{full_path} #{full_path.gsub('.png', '.thumb.jpg')}`
    end
    ...
    $logger.info "[#{id}] Finished syncing file #{sfile.dropbox_path}"
